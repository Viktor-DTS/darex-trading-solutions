...
module.exports = {
  ...
  posConvDecay: num('FX_POS_CONV_DECAY', charlieMode ? 22 : 15),
  posConvDecayLoss: num('FX_POS_CONV_DECAY_LOSS', charlieMode ? 25 : 8),
  /** Min age before underwater conv_decay can fire (debate: winners ~8m). */
  posConvDecayMinAgeMs: num('FX_POS_CONV_DECAY_MIN_AGE_MS', charlieMode ? 360000 : 0),
  ...
  posTimeScratchMs: num('FX_POS_TIME_SCRATCH_MS', charlieMode ? 900000 : 300000),
  /** 0 = disable loss time_scratch (debate 03.08). */
  posTimeScratchLossMs: num('FX_POS_TIME_SCRATCH_LOSS_MS', charlieMode ? 0 : 180000),
  ...
  pairDayLossCap: num('FX_PAIR_DAY_LOSS_CAP', 2),
  longBias: process.env.FX_LONG_BIAS !== '0',
  shortMaxPUp: num('FX_SHORT_MAX_P_UP', 0.35),
  shortMinKappa: num('FX_SHORT_MIN_KAPPA', 0.55),
  charlieAsianSkip: process.env.FX_CHARLIE_ASIAN_SKIP !== '0',
  charlieAsianSkipStartUtc: str('FX_CHARLIE_ASIAN_SKIP_START', '03:00'),
  charlieAsianSkipEndUtc: str('FX_CHARLIE_ASIAN_SKIP_END', '04:00'),
  ...
  mathMicroMinBarsInStop: num('FX_MATH_MICRO_BARS', 0.35),
  ...
  testbot: {
    ...
    /** Early bank green (debate 03.08: bigger bank, rarer cut). */
    earlyPartialUsd: num('FX_TESTBOT_EARLY_PARTIAL_USD', 2.2),
    earlyPartialMs: num('FX_TESTBOT_EARLY_PARTIAL_MS', 240000),
    protectPeakUsd: num('FX_TESTBOT_PROTECT_PEAK_USD', 2),
    protectFloorUsd: num('FX_TESTBOT_PROTECT_FLOOR_USD', 1),
    holdExtendMs: num('FX_TESTBOT_HOLD_EXTEND_MS', 300000),
    /** CUT_STALE: mid-hold underwater without early peak → cut before full SL. */
    cutStaleMs: num('FX_TESTBOT_CUT_STALE_MS', 720000),
    cutStaleUsd: num('FX_TESTBOT_CUT_STALE_USD', 2.5),
    pairDayLossCap: num('FX_TESTBOT_PAIR_DAY_LOSS_CAP', 2),
    pairBlacklist: parsePairList(process.env.FX_TESTBOT_PAIR_BLACKLIST || process.env.FX_PAIR_BLACKLIST || ''),
    /** London bank window (debate 04.08): default 11:00–15:00 UTC. */
    sessionGate: process.env.FX_TESTBOT_SESSION_GATE !== '0',
    sessionStartUtc: str('FX_TESTBOT_SESSION_START_UTC', '11:00'),
    sessionEndUtc: str('FX_TESTBOT_SESSION_END_UTC', '15:00'),
    longBias: process.env.FX_LONG_BIAS !== '0',
    shortMaxPUp: num('FX_SHORT_MAX_P_UP', 0.35),
    shortMinKappa: num('FX_SHORT_MIN_KAPPA', 0.55),
    /**
  ...
  oracle: {
    ...
    toxicWr: num('FX_ORACLE_TOXIC_WR', 0.35),
    toxicMinTrades: num('FX_ORACLE_TOXIC_MIN_TRADES', 5),
    minStatsSamples: num('FX_ORACLE_MIN_STATS_SAMPLES', 30),
    minDirectionHitRate: num('FX_ORACLE_MIN_DIRECTION_HIT', 0.52),
    minPairHitRate: num('FX_ORACLE_MIN_PAIR_HIT', 0.55),
