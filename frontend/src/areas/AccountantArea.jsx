...
const AccountantArea = memo(function AccountantArea({ user, accessRules, currentArea }) {
  ...
  // Використовуємо хук useLazyData для оптимізації (як у AccountantApprovalArea)
  const { 
    data: tasks, 
    loading, 
    error, 
    ...
    refreshData, 
    getTabCount 
  } = useLazyData(user, 'invoiceRequests');
  ...
  useEffect(() => {
    const loadColumnSettings = async () => {
      try {
        const settings = await columnsSettingsAPI.loadSettings(user.login, 'accountant');
  ...
  const uploadInvoiceFile = async (requestId, file) => {
    try {
      ...
      console.log('DEBUG AccountantArea: API_BASE_URL:', API_BASE_URL);
      
      // Конвертуємо PDF в JPG якщо потрібно
      console.log('DEBUG AccountantArea: Викликаємо processFileForUpload...');
  ...
  const handleApprove = async (id, approved, comment) => {
    try {
      ...
      await refreshData(activeTab);
      // Видалено setTableKey для оптимізації // Примусово перерендерюємо таблицю
  ...
  const handleSaveColumns = async (selectedColumns) => {
    try {
      ...
      console.log('[DEBUG] AccountantArea - збережено налаштування колонок:', selectedColumns.length);
    } catch (error) {
  ...
  useEffect(() => {
    const loadAllTasks = async () => {
      ...
      try {
        ...
        if (taskKV245OnLoad) {
          console.log('[DEBUG KV-0000245] AccountantArea - заявка знайдена при завантаженні з API:', taskKV245OnLoad);
        } else {
          console.log('[DEBUG KV-0000245] AccountantArea - заявка НЕ знайдена при завантаженні з API');
        }
      } catch (error) {
  ...
  const debt = allTasksFromAPI.filter(task => {
    ...
    if (task.requestNumber === 'KV-0000245' || task.taskNumber === 'KV-0000245') {
      console.log('[DEBUG KV-0000245] AccountantArea - початок фільтрації:', {
    ...
    // Логування для конкретної заявки KV-0000245
    if (task.requestNumber === 'KV-0000245' || task.taskNumber === 'KV-0000245') {
      console.log('[DEBUG KV-0000245] AccountantArea - результат фільтрації:', {
  ...
  if (taskKV245) {
    console.log('[DEBUG KV-0000245] AccountantArea - заявка знайдена в allTasksFromAPI:', taskKV245);
  } else {
    console.log('[DEBUG KV-0000245] AccountantArea - заявка НЕ знайдена в allTasksFromAPI');
  }
  
  // Перевірка чи є заявка KV-0000245 в debt
  const debtKV245 = debt.find(t => t.requestNumber === 'KV-0000245' || t.taskNumber === 'KV-0000245');
  if (debtKV245) {
    console.log('[DEBUG KV-0000245] AccountantArea - заявка знайдена в debt:', debtKV245);
  } else {
    console.log('[DEBUG KV-0000245] AccountantArea - заявка НЕ знайдена в debt');
  }
  ...
  }, [activeTab, filteredInvoiceRequests]);
  
  // Логування для діагностики tableData
  console.log('[DEBUG] AccountantArea - tableData оновлено:', {
    activeTab,
    tableDataLength: tableData.length,
    filteredInvoiceRequestsLength: filteredInvoiceRequests.length,
    tasksLength: tasks.length
  });
