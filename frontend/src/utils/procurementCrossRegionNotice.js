/** Попередження про переміщення між регіонами (завсклад, прийом VZ). */
...
function buildWarehouseRegionMap(warehouses) {
  const map = new Map();
...
export function computeProcurementCrossRegionNotices(pr, warehouses) {
  const desired = String(pr?.desiredWarehouse || '').trim();
  if (!desired) return [];

  const regionMap = buildWarehouseRegionMap(warehouses);
