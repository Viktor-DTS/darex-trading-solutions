...
export async function getShipmentRequestPreviewNumber() {
  const res = await authFetch(`${API_BASE_URL}/shipment-requests/preview-number`, {
    headers: authHeaders()
  });
...
export async function submitSaleShipmentRequest(saleId, payload, ttnFile) {
  ...
  const res = await authFetch(`${API_BASE_URL}/sales/${saleId}/shipment-request`, {
    method: 'POST',
    headers: authHeaders(),
    body: formData
  });
...
export async function getShipmentRequestFull(id) {
  const res = await authFetch(`${API_BASE_URL}/shipment-requests/${id}`, {
    headers: authHeaders()
  });
  if (res.status === 401) return null;
...
export async function listShipmentRequests(status = 'pending') {
  const res = await authFetch(`${API_BASE_URL}/shipment-requests?status=${encodeURIComponent(status)}`, {
    headers: authHeaders()
  });
  if (res.status === 401) return [];
...
export async function patchShipmentRequestStatus(id, status) {
  const res = await authFetch(`${API_BASE_URL}/shipment-requests/${id}/status`, {
    method: 'PATCH',
