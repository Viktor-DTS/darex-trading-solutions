...
function AdminDashboard({ user }) {
  ...
  const NOTIFICATION_TYPES = [
    ...
    { key: 'warehouseTransferRequested', label: 'Переміщення: новий запит' },
    { key: 'warehouseTransferApproved', label: 'Переміщення: підтверджено відправку' },
    { key: 'warehouseTransferReceived', label: 'Переміщення: прийнято на складі' },
    { key: 'warehouseTransferRejected', label: 'Переміщення: відхилено' },
        ...
        <>
          <div className="notifications-info">
            ...
            <p className="notifications-info-hint">
              Колонки «VZ: …» — Telegram про заявки закупівель.
              Адміністратор отримує типи лише з увімкненим чекбоксом у цьому рядку (без обходу через TELEGRAM_ADMIN_CHAT_ID).
              «VZ: нова заявка» — усі з чекбоксом (відділ закупівель бачить заявки від усіх).
              Інші колонки VZ: після чекбокса є фільтр по заявці — отримує той, хто її подав, плюс адміністратор (лише з чекбоксом). Чужі заявки менеджерам не розсилаються.
              ...
              «VZ: відхилено» — заявник завжди; адміністратор — лише з чекбоксом.
              Колонки «Переміщення: …» — запити сервісу на переміщення між складами. Адміністратор отримує їх лише з увімкненим чекбоксом. Завсклади регіону джерела/отримувача та ініціатор запиту отримують операційні сповіщення незалежно від цих колонок.
            </p>
          </div>
