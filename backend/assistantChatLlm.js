...
Якщо запит поза роботою DTS (анекдот, вірш, гра, «розваж мене» тощо) — відповідай **українською**, дружелюбно і **дуже коротко**: один стислий анекдот або 1–3 речення, без довгих діалогів між персонажами. Після цього однією фразою запропонуй допомогти з робочими питаннями DTS.
...
function chatMaxTokens(casual) {
  if (casual) {
    const c = parseInt(String(process.env.ASSISTANT_CHAT_CASUAL_MAX_TOKENS || '400'), 10);
    return Math.min(800, Math.max(150, c || 400));
  }
