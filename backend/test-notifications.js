...
// Підключаємо схеми з основного файлу
require('./index.js');
...
async function testNotificationSystem() {
  try {
    ...
    const userAnalysis = await notificationDebugger.analyzeUserSettings();
    ...
    const logAnalysis = await notificationDebugger.analyzeNotificationLogs(20);
    ...
    for (const testCase of testCases) {
      ...
      const result = await notificationDebugger.simulateNotification(testCase.type, testCase.data);
    ...
    if (userWithTelegram) {
      ...
      const testResult = await notificationDebugger.testUserNotification(
