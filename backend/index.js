...
const PROGRAMMATIC_SERVICE_COEFFICIENTS = [
  {
    ...
    label: 'Базовий відсоток за виконану роботу',
    defaultValue: 25,
    note:
  ...
  },
  {
    ...
    defaultValue: 25,
    note:
      'Та сама формула, що й «Базовий відсоток за виконану роботу» (від ціни робіт). Застосовується лише з 01.05.2026 за датою проведення робіт і лише для найменувань: Ремонт в цеху, Ремонт на місці, Діагностика+ремонт, Ремонт в цеху (волонтерство). Для таких заявок базовий відсоток не використовується.'
  },
  {
...
app.patch('/api/procurement-requests/:id/confirm-documents', async (req, res) => {
  ...
  try {
    ...
    const out = await ProcurementRequest.findById(pr._id).select(PROCUREMENT_DOC_LIST_PROJECTION).lean();
    stripProcurementLineBinaryFields(out);
    await dispatchProcurementTelegram('request_completed', out);
...
app.get('/api/inventory-movement-log', authenticateToken, async (req, res) => {
  try {
    ...
    if (isBczvsklRole(req.user.role)) {
      ...
      const scope = await collectBczvsklWarehouseScope(Warehouse, lookup);
      internalFilter = buildBczvsklInternalLogFilter(scope);
      onecFilter = buildBczvsklOneCMovementFilter(scope, mongoose);
  ...
  try {
    ...
    if (scope === 'service') {
      ...
      if (svcPct && svcPct.value <= 0) {
        return res.status(400).json({
          error:
            '«Базовий відсоток за виконану роботу» має бути більшим за 0 (введіть відсоток, наприклад 25).'
        });
      }
...
app.get('/api/warehouses', authenticateToken, async (req, res) => {
  ...
  try {
    ...
    if (isBczvsklRole(req.user.role)) {
      const lookup = await buildOneCWarehouseLookup(Warehouse, OneCWarehouseAlias);
      warehouses = forMoveDest ? [] : await loadBczvsklAllowedWarehouses(Warehouse, lookup);
...
const { getProductCardLlmStatus } = require('./productCardAssistantLlm');
...
app.get('/api/onec/movements', async (req, res) => {
  try {
    ...
    const bczvsklWarehouseScope = bczvsklScope
      ? await collectBczvsklWarehouseScope(
          Warehouse,
          await buildOneCWarehouseLookup(Warehouse, OneCWarehouseAlias)
        )
      : null;
    const bczvsklAllowedIds = bczvsklWarehouseScope ? bczvsklWarehouseScope.allowedIds : null;
    ...
    const lookup = await buildOneCWarehouseLookup(Warehouse, OneCWarehouseAlias);
    if (bczvsklScope) {
      filter = mergeMongoFilters(filter, buildBczvsklOneCMovementFilter(bczvsklWarehouseScope, mongoose));
...
async function buildEquipmentListQuery(req) {
  ...
  if (isBczvsklRole(req.user?.role)) {
    ...
    scopeOr.push({ currentWarehouseName: { $regex: BCZVSKL_WAREHOUSE_RX_SOURCE, $options: 'i' } });
    const cw = query.currentWarehouse;
    if (cw != null) {
      const requested = cw.$in ? cw.$in.map(String) : [String(cw)];
      const filtered = requested.filter((id) => allowedIds.has(String(id)));
      query.currentWarehouse = {
        $in: filtered.filter((id) => mongoose.isValidObjectId(id)).map((id) => new mongoose.Types.ObjectId(id)),
      };
    } else {
...
app.get('/api/onec/pending-move-receipts', authenticateToken, async (req, res) => {
  try {
    ...
    const activeItems = items.filter((i) => !i.isConfirmed);
    const incomingCount = activeItems.filter((i) => i.canConfirm).length;
    const outgoingCount = activeItems.filter((i) => i.receiptSide === 'outgoing').length;
    res.json({
      items,
      total: items.length,
      activeCount: activeItems.length,
      historyCount: items.length - activeItems.length,
      incomingCount,
      outgoingCount,
