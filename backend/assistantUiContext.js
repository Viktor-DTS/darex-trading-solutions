...
function formatAssistantSessionBlock({ client, serverUser, login }) {
  ...
  lines.push('');
  lines.push(
    'Правила поведінки: орієнтуй підказки на поточну панель із [DTS/UI] — не описуй екран іншої вкладки як «те, що перед користувачем». Якщо людина не на сервісі, а питає про операції з KV-таблицею — поясни різницю та куди перемкнутися (найчастіше «Сервісна служба» або «Оператор»). Якщо у повідомленні згадано номер заявки, але серверне підстановлення даних відсутнє або блок [DTS] каже недоступно — не вигадуй статус, суми чи склад; коротко поясни можливі причини (регіон, закріплені клієнти менеджера, відсутність у призначених інженерах).',
  );
